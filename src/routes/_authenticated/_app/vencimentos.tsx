...
function DeadlinesPage() {
  const { data } = useQuery({ queryKey: ["vencimentos-agenda"], queryFn: async () => { const [v, a, cars] = await Promise.all([supabase.from("vencimentos").select("*").order("data_vencimento"), supabase.from("agenda_eventos").select("*").order("data"), supabase.from("veiculos").select("id, placa")]); const err = v.error || a.error || cars.error; if (err) throw err; return { vencimentos: v.data ?? [], agenda: a.data ?? [], plates: new Map((cars.data ?? []).map(c => [c.id, c.placa])) }; } });
  ...
  return <><PageHeader title="Vencimentos e Agenda" description="Controle de prazos e compromissos operacionais." /><Tabs defaultValue="vencimentos"><TabsList><TabsTrigger value="vencimentos"><ListFilter className="mr-2 size-4" />Vencimentos</TabsTrigger><TabsTrigger value="agenda"><CalendarDays className="mr-2 size-4" />Agenda</TabsTrigger></TabsList><TabsContent value="vencimentos"><DataPage rows={data?.vencimentos ?? []} columns={deadlineCols} /></TabsContent><TabsContent value="agenda"><DataPage rows={data?.agenda ?? []} columns={agendaCols} /></TabsContent></Tabs></>;
}