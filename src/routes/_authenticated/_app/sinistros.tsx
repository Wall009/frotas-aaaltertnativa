...
function ClaimsPage() {
  const { data } = useQuery({ queryKey: ["sinistros"], queryFn: async () => { const [rows, cars] = await Promise.all([supabase.from("sinistros").select("*").order("data", { ascending: false }), supabase.from("veiculos").select("id, placa, status").order("placa")]); if (rows.error || cars.error) throw rows.error || cars.error; const allVehicles = cars.data ?? []; return { rows: rows.data ?? [], vehicles: allVehicles.filter(c => c.status !== "VENDIDO"), plates: new Map(allVehicles.map(c => [c.id, c.placa])) }; } });
